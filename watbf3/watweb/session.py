...
class BaseSession(collections.MutableMapping):
	...
	def refresh(self, duration=None, new_session_id=False): # the opposite of invalidate
		...
		# self.save() # don't save in refresh, wait for 
...
try:
	...
	class MongoDBSession(BaseSession):
		...
		def __init__(self, db, **kwargs):
			BaseSession.__init__(self, **kwargs)
			self.db = db
		...
		def delete(self):
			...
			yield self.db.remove({'session_id': self.session_id})

	@defer.inlineCallbacks
	def delete_expired(db):
		yield db.remove({'expires': {'$lte': int(time.time())}})

	@defer.inlineCallbacks
	def load(session_id, db):
		"""Load session from the storage."""
		data = yield db.find_one({'session_id': session_id})
		if data:
			kwargs = MongoDBSession.deserialize(data['data'])
			defer.returnValue(MongoDBSession(db, **kwargs))
